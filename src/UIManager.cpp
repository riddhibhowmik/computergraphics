...
bool UIManager::FloatBox(Rectangle bounds, float* value, int id) {
    ...
    if (isHovered && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        activeTextBox = -1;
        try { *value = std::stof(textInputBuffer); } catch (...) {}
    }
    ...
    if (activeTextBox == id) {
        ...
        if (IsKeyPressed(KEY_BACKSPACE) && !textInputBuffer.empty()) textInputBuffer.pop_back();
...
bool UIManager::IntBox(Rectangle bounds, int* value, int id) {
    ...
    if (isHovered && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        activeTextBox = -1;
        try { *value = std::stoi(textInputBuffer); } catch (...) {}
    }
...
void UIManager::DrawSidebar(Project& currentProject) {
    ...
    if (Button({1920 - 480 + 20, 150, 440, 40}, "+ Add custom script")) {
    ...
    if (currentProject.selectedEffectIndex >= 0 && currentProject.selectedEffectIndex < currentProject.activeEffects.size()) {
        ...
        if (auto snow = std::dynamic_pointer_cast<SnowEffect>(effect)) {
            ...
            if (activeTextBox != 4) snow->seed = (int)seedFloat; // Only update from slider if not typing
            ...
            if (Button({1920 - 480 + 20, 560, 100, 30}, "< Prev")) {
                snow->easingType--;
                if (snow->easingType < 0) snow->easingType = (int)EasingType::Count - 1;
            }
            if (Button({1920 - 480 + 130, 560, 100, 30}, "Next >")) {
                snow->easingType++;
                if (snow->easingType >= (int)EasingType::Count) snow->easingType = 0;
            }
            ...
            if (Button({1920 - 480 + 200, 665, 80, 30}, "Toggle")) snow->isActive = !snow->isActive;
        }
    } else {
